...
enum sprdbat_event {
	...
	//changed begin
	SPRDBAT_CHG_FULL_99_TIMEOUT_E,
	//changed end
	SPRDBAT_CHG_UNSPEC_E,
};
//changed begin
struct timespec  time_99_percent;
int time_period_99pecent = 0;
//changed end
...
#endif
	SPRDBAT_CALIBERATE_ATTR_WO(chg_temp_debug),
...
static int temp_debug_enable = 0;
static int temp_debug = 200;
					...
					const char *buf, size_t count)
{
	...
	case CHG_TEMP_DEBUG_EN:
		if(set_value > 4000)
			temp_debug_enable = 0;
		else
			temp_debug_enable = 1;
		temp_debug = set_value;
		break;
...
static int sprdbat_start_charge(void)
{
	...
	if((thermal_ctrl_chg_cur) &&( adjust_chg_flag)){
		sprdbat_thermal_ctrl_chgcurrent(cool_current);
	}
...
static void sprdbat_change_module_state(uint32_t event)
{
	...
	case SPRDBAT_CHG_UNSPEC_E:
		...
		break;
	//changed begin
	//set re-charge timeout and set status to full
	case SPRDBAT_CHG_FULL_99_TIMEOUT_E:
...
static int plugin_callback(void)
{
	...
	//changed begin
	//current percent is 99%
	if(99==sprdbat_data->bat_info.capacity){
		time_period_99pecent = 1;
		get_monotonic_boottime(&time_99_percent);
	}
	//changed end
...
static void sprdbat_update_capacty(void)
{
	...
	if (fgu_capacity != sprdbat_data->bat_info.capacity) {
		...
		power_supply_changed(&sprdbat_data->battery);
		//changed begin
		if(99==sprdbat_data->bat_info.capacity &&POWER_SUPPLY_STATUS_CHARGING==sprdbat_data->bat_info.module_state){
			time_period_99pecent = 1;
			get_monotonic_boottime(&time_99_percent);
		}
		//changed end
...
///fengnan add spreadtrum path @20170307 for OTG charge
//pengwei add begin
#define OVER_TEMP_OTG		500
#define LOW_CAP_OTG			6
static int temp_flag = 0;
static int cap_flag = 0;

int sprdbat_otg_en_cap_temp(void)
{

/*bsp_baoshulin_del_begin: OTG add the switch node : 20170904*/
#if 0
		int temp;

		temp = sprdbat_read_temp();
		cap = sprdbat_data->bat_info.capacity;
		SPRDBAT_DEBUG("sprdbat_otg_en_cap_temp:  %d\n",cap);
		if(temp > OVER_TEMP_OTG){
			temp_flag ++;
		}
		else{
			temp_flag = 0;
...
/*bsp_baoshulin_del_begin: OTG add the switch node : 20170904*/
		...
		SPRDBAT_DEBUG("sprdbat_otg_en_cap_temp:  cap_flag=%d,temp_flag=%d\n",cap_flag,temp_flag);
		if(cap_flag >= 2){
				cap_flag = 2;
				//temp_flag = 2;
				SPRDBAT_DEBUG("sprdbat_otg_en_cap_temp:  set disable otg\n");
				return 1;
		}
		return 0;
}
#ifndef  CONFIG_SPRD_USB_DEVICE_ONLY      //fengnan add @20170309 for otg
extern void charge_ctr_host_mode();
/*bsp_baoshulin_add_begin: OTG add the switch node : 20170904*/
extern int start_otg_flag;
/*bsp_baoshulin_add_end: OTG add the switch node : 20170904*/
void sprdbat_otg_en(void)
{
		if(sprdbat_otg_en_cap_temp() && start_otg_flag == 1){
			charge_ctr_host_mode(0);
		}else{
			/*bsp_baoshulin_del_begin: OTG add the switch node : 20170904*/
			//charge_ctr_host_mode(1);
			/*bsp_baoshulin_del_end: OTG add the switch node : 20170904*/
		}
}
#else
void sprdbat_otg_en(void)
...
//pengwei add end
///end of fengnan add spreadtrum path @20170307 for OTG charge
...
///end of fengnan add spreadtrum path @20170307 for OTG charge
	...
	if(temp_debug_enable)
	    if(temp_debug >2000)
		{
			temp_debug = 2000 - temp_debug;
		    sprdbat_data->bat_info.cur_temp = temp_debug;
		}
        else
        {
		    sprdbat_data->bat_info.cur_temp = temp_debug;
...
#endif
//changed begin
static void percent_99_timeout_monitor()
{
	struct timespec  curr_time;
	if(time_period_99pecent){
		get_monotonic_boottime(&curr_time);
		if(curr_time.tv_sec - time_99_percent.tv_sec>900){
			sprdbat_change_module_state(SPRDBAT_CHG_FULL_99_TIMEOUT_E);
			time_period_99pecent = 0;
		}
		}
}
